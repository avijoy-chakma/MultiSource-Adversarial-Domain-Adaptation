...
gpu_id= 0
activities = ['lying', 'sitting', 'standing', 'walking', 'running', 'cycling', 'ascending', 'descending', 'vacuum', 'ironing', 'rope_jumping']
...
dataset_path = home_url+"/Downloads/ISWC/Preprocessing/PAMAP/Data Files/"+folder_name+"/"

plot_save_path = os.getcwd() + "/MSADA/PAMAP-SameUser-DifferentPosition-Augmented-11/"+folder_name+"/Target-Position-"+position_array[data_source["target"]]+"/User "+ str(selected_user)+"/"

pretrain_path = "/notebooks/Downloads/ISWC/Debug-Pretraining/Pre-trained/PAMAP-SameUser-DifferentPosition-Augmented-11/"+folder_name+"/Target-Position-"+position_array[data_source["target"]]+"/User "+ str(selected_user)+"/"
...
experiment_summery_file = os.getcwd() + "/MSADA/PAMAP-SameUser-DifferentPosition-Augmented-11/"
file1 = open(experiment_summery_file+"PAMAP-SameUser-DifferentPosition-Augmented-11.txt", "a")
...
for step in tqdm.tqdm(range(steps)):
    ...
    for i, (Modified_t_sample, t_sample, t_labels) in tqdm.tqdm(enumerate(t_loader)):
        ...
        # Do we need to augment the target samples?
        
#         print("S1 Sample shape: " + str((s1_sample.shape)) + " Modified Sample shape: "+str((Modified_s1_Sample.shape)))
#         print("S2 Sample shape: " + str((s2_sample.shape)) + " Modified Sample shape: "+str((Modified_s2_Sample.shape)))
        Modified_s1_Sample = Variable(Modified_s1_Sample.float().cuda(gpu_id))
        Modified_s2_Sample = Variable(Modified_s1_Sample.float().cuda(gpu_id))
...
#         target_running_accuracy = target_current_accuracy
    ...
    if step == steps-1:
        ...
        file1.write(person_list[selected_user-1] +" "+position_array[source1] +" "+ person_list[selected_user-1] +" "+ position_array[source2] +" "+ person_list[selected_user-1] +" "+ position_array[target] +" "+ str("%.4f"%target_accuracy) +" "+ str("%.4f"%f1_micro) +" "+ str("%.4f"%precision_micro) +" "+ str("%.4f"%recall_micro)+"\n")
        file1.close()
    
         
# new_accuracy_plot(selected_user, plot_save_path, source1_running_accuracy, source2_running_accuracy, target_running_accuracy)
# new_loss_plot(selected_user, plot_save_path)
